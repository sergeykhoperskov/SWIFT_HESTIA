...
INLINE static int star_formation_should_spawn_spart(
  ...
  return starform->n_stars_per_sf_event;
}
...
INLINE static void star_formation_copy_properties(
  ...
  sp->mass = hydro_get_mass(p)/(starform->n_stars_per_sf_event);

  /* Store the current mass as the initial mass */
  sp->mass_init = hydro_get_mass(p)/(starform->n_stars_per_sf_event);

  sp->gpart->mass = hydro_get_mass(p)/(starform->n_stars_per_sf_event);
...
INLINE static void starformation_init_backend(
  ...
  starform->n_stars_per_sf_event = parser_get_param_int(
        parameter_file, "EAGLEStarFormation:n_stars_per_sf_event");
...
INLINE static void starformation_print_backend(
  ...
  message("Star formation will create %d star particles per SF event",starform->n_stars_per_sf_event);
  switch (starform->SF_threshold) {
    case eagle_star_formation_threshold_Z_dep:
